...
The following assertions compareAbsolute two values. The value arguments must be
...
See also the [Floating-Point Comparison](#floating-point) assertions to compareAbsolute
...
[`EXPECT_STREQ`](#EXPECT_STREQ) to compareAbsolute C strings (e.g. `const char*`) by
value.
...
[`EXPECT_STRNE`](#EXPECT_STRNE) to compareAbsolute C strings (e.g. `const char*`) by
value.
...
The following assertions compareAbsolute two **C strings**. To compareAbsolute two `string`
objects, use [`EXPECT_EQ`](#EXPECT_EQ) or [`EXPECT_NE`](#EXPECT_NE) instead.

These assertions also accept wide C strings (`wchar_t*`). If a comparison of two
wide strings fails, their values will be printed as UTF-8 narrow strings.

To compareAbsolute a C string with `NULL`, use `EXPECT_EQ(`*`c_string`*`, nullptr)` or
...
The following assertions compareAbsolute two floating-point values.
